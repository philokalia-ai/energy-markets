...
function _create_multi_zone_order_book_merit(zones::Vector{String}, day::Date;
    ...
    function build_zone_book(zone::String, exclude::Vector{String})
        ...
        # cv34: per-UTC-hour import capacity into `zone` over its ENDOGENOUS
        # borders as the enriched network (hence the MPCC) has it, for the
        # import backstop sizing (see get_import_backstop). Periods are hour
        # strings "1".."24".
        endo_atc = Dict{Int,Float64}()
        if enrich_network && isempty(get(ENV, "EUPHEMIA_DISABLE_BACKSTOP_ATC_SYNC", ""))
        ...
        return create_merit_order_book(zone, day;
            ...
            endogenous_import_atc=(enrich_network &&
                isempty(get(ENV, "EUPHEMIA_DISABLE_BACKSTOP_ATC_SYNC", ""))) ? endo_atc : nothing,
