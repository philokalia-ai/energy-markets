...
# cv34 leave-one-out switch: EUPHEMIA_DISABLE_CV34_D2CLOSE set -> the cv33
# D-1 close (lag 0 days before D). Read at call time (worker-safe).
_gate_lag_days() = isempty(get(ENV, "EUPHEMIA_DISABLE_CV34_D2CLOSE", "")) ? 1 : 0
...
function get_daily_eua_price(day::Dates.Date)
    ...
    df = try
        Euphemia.sql2df_with_retry(
            ...
            WHERE date < \$1::date - INTERVAL '$(_gate_lag_days()) day' AND date > \$1::date - INTERVAL '11 days'
...
# (`_boundary_anchor(:gb_ccgt_srmc)` with `carbon_source=:uka`) uses this instead
# of the EUA proxy — GB left the EU ETS and its CCGT fleet pays the UK price,
# which has diverged from EUA (mid-2026: UKA ≈ €66 vs EUA ≈ €79). Cached per day
# like EUA/TTF; genuine absence cached, transient errors never cached.
...
function uka_price(day::Dates.Date)
    ...
    try
        df = Euphemia.sql2df(
            ...
            WHERE price_date < \$1::date - INTERVAL '$(_gate_lag_days()) day' AND price_eur IS NOT NULL
