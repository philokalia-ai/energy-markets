...
% =============================================================================
\section{Βιβλίο Εντολών Οριακού Κόστους (Merit-Order)}
\label{sec:merit-order-book}

Πέραν των στρατηγικών που βασίζονται στη λύση του Unit Commitment, το σύστημα υλοποιεί (module \texttt{MeritOrderBook.jl}) μια ντετερμινιστική μέθοδο κατασκευής βιβλίου εντολών απευθείας από τα θεμελιώδη μεγέθη της αγοράς, η οποία αναδείχθηκε ως η ακριβέστερη (Κεφάλαιο 5) και αποτελεί την προεπιλεγμένη μέθοδο \texttt{:merit\_order}. Η κεντρική ιδέα είναι η κατασκευή ενός \emph{ανταγωνιστικού αντιπαραδείγματος}: κάθε μονάδα προσφέρει στο βραχυχρόνιο οριακό της κόστος (SRMC) συν την ορθολογική --- όχι όμως στρατηγική --- συμπεριφορά που επιβάλλουν τα τεχνικά και οικονομικά χαρακτηριστικά της. Η ενότητα αυτή περιγράφει τον \emph{σκελετό προσφορών} (bid skeleton): πώς ακριβώς κατασκευάζεται η προσφορά κάθε μονάδας, και γιατί η διαδρομή αυτή σκόπιμα \emph{δεν} περνά από πλήρη επίλυση Unit Commitment.\footnote{Η σύνθεση του σκελετού προσφορών τεκμηριώνεται και στο έγγραφο \texttt{docs/calibration-atlas.md} (§3) του αποθετηρίου.}

\subsection{Γιατί Όχι Πλήρες Unit Commitment}
\label{subsec:why-not-uc}

Το ζητούμενο του αντιπαραδείγματος είναι η \emph{ορθολογική ανταγωνιστική προσφορά} κάθε μονάδας --- όχι μια προσομοιωμένη κεντρική κατανομή. Το πλήρες Unit Commitment απαντά σε διαφορετική ερώτηση («τι θα έτρεχε ένας κεντρικός σχεδιαστής»), και η μετατροπή της λύσης του σε εντολές αποδείχθηκε εκφυλισμένη: η προσφερόμενη ποσότητα ταυτίζεται με τη ζήτηση και η τιμή καθηλώνεται στην ακριβότερη \emph{δεσμευμένη} μονάδα, αντί να προκύπτει από τον ανταγωνισμό ολόκληρης της καμπύλης προσφοράς (βλ.\ και Πίνακα~\ref{tab:driver-analysis}). Επιπλέον, το UC είναι περίπου 100 φορές βραδύτερο --- απαγορευτικό για εκκαθάριση 39 ζωνών σε δύο περάσματα. Το μόνο στοιχείο λογικής κατανομής που διατηρείται είναι η ελαφριά δέσμευση της επόμενης υποενότητας.

\subsection{Ελαφριά Δέσμευση (UC-lite)}
\label{subsec:uc-lite}

Αντί πλήρους βελτιστοποίησης, η δέσμευση των θερμικών μονάδων προσδιορίζεται με έναν ντετερμινιστικό κανόνα στοίβαξης: οι θερμικές μονάδες με $\text{SRMC} \le 1{,}15 \times \text{SRMC}_{\text{αερίου}}$ ταξινομούνται κατά αύξον οριακό κόστος και στοιβάζονται έως ότου η (απομειωμένη) ικανότητά τους καλύψει το $1{,}05 \times$ της \emph{μέγιστης} ημερήσιας καθαρής ζήτησης. Το σύνολο αυτό θεωρείται «δεσμευμένο» --- και επειδή η δέσμευση ακολουθεί την αιχμή, το τεχνικό ελάχιστο των μονάδων αυτών προσφέρεται ως αυτοπρογραμματισμένο (must-run) και στις νυχτερινές ώρες χαμηλής ζήτησης. Αυτός ακριβώς ο μηχανισμός επιτρέπει στις τιμές να καταρρέουν \emph{κάτω} από το θερμικό SRMC τις ώρες πλεονάσματος ΑΠΕ, όπως συμβαίνει με τον πραγματικό αυτοπρογραμματισμό.

\subsection{Ο Σκελετός Προσφορών ανά Κατηγορία Μονάδας}
\label{subsec:bid-skeleton}

Κάθε βιβλίο ζώνης-διαστήματος χρησιμοποιεί τον ίδιο σκελετό· οι περιοχές αλλάζουν μόνο παραμέτρους. Ο Πίνακας~\ref{tab:bid-skeleton} συνοψίζει τι προσφέρει κάθε κατηγορία μονάδας και με ποιον κανόνα τιμής.

\begin{table}[htbp]
\centering
\caption{Ο σκελετός προσφορών: ποσότητα και κανόνας τιμής ανά κατηγορία μονάδας}
\label{tab:bid-skeleton}
\begin{tabular}{@{}p{3.4cm}p{4.3cm}p{5.6cm}@{}}
\toprule
\textbf{Κατηγορία} & \textbf{Προσφερόμενη ποσότητα} & \textbf{Κανόνας τιμής} \\
\midrule
Αιολικά / φωτοβολταϊκά & Ζωνική πρόβλεψη (οι μονάδες εξαιρούνται από τη στοίβα) & 1~€/MWh --- δέκτης τιμής \\
Υδροηλεκτρικά ταμιευτήρα / αντλησιοταμίευση & $p_{\max}$ επί συντελεστή κλίμακας (πληρότητα ταμιευτήρων / πρόσφατη παραγωγή) & \textbf{Αξία νερού} (τρία μοντέλα: αγκυρωμένη στο αέριο, ευκαιρίας ταμιευτήρα, ευκαιρίας με άγκυρα)· ποτέ το μεταβλητό κόστος \\
Δεσμευμένη θερμική --- τεχνικό ελάχιστο & $p_{\min}$ σε δύο τμήματα & Το βαθύτερο 60\% στο 5\% του SRMC· το υπόλοιπο στο $\max(0{,}5 \cdot \text{SRMC},\ \text{SRMC} - 40)$ \\
Ίδια μονάδα --- ευέλικτη ισχύς & $(p_{\max} - p_{\min})$ σε βαθμίδες 55/20/15/10\% & Κλίμακα $0{,}95 / 1{,}05 / 1{,}25 / 1{,}60 \times \text{SRMC}$· η πρώτη βαθμίδα στο κόστος, οι ανώτερες επί τον όρο στενότητας \\
...
Δύο κανόνες του πίνακα αξίζουν σχολιασμό. Πρώτον, η έκπτωση του τεχνικού ελαχίστου είναι \emph{απόλυτο} μέγεθος ($\text{SRMC} - 40$~€/MWh, με κατώφλι το μισό SRMC) και όχι ποσοστό: η έκπτωση αυτή αποσβένει κόστος εκκίνησης σε €/MWh, οπότε η αναλογική διατύπωση, αόρατη σε κανονικά έτη, κατέρρεε στα βράδια της κρίσης του 2022 (βλ.\ Ενότητα~\ref{subsec:crisis-2022}). Δεύτερον, ο όρος στενότητας εφαρμόζεται μόνο στις ανώτερες βαθμίδες και έχει τη μορφή
\begin{equation}
\sigma_t = 1 + \kappa_s \cdot \max(0,\ \theta - m_t)^2 + \kappa_p \cdot \hat{d}_t^{\,4},
\label{eq:scarcity-factor}
\end{equation}
όπου $m_t$ το περιθώριο απομειωμένης ικανότητας πάνω από την καθαρή ζήτηση, $\theta$ το κατώφλι στενότητας και $\hat{d}_t$ η κανονικοποιημένη ζήτηση. Ο πρώτος όρος ενεργοποιείται όταν το περιθώριο \emph{πραγματικά} στενεύει· ο δεύτερος αποτυπώνει τη στρατηγική προσφορά των ωρών αιχμής (οι συμμετέχοντες γνωρίζουν πότε είναι η αιχμή, και η τέταρτη δύναμη συγκεντρώνει το πριμ εκεί).

\subsection{Πιστότητα Στόλου}
\label{subsec:fleet-honesty}

Πριν από κάθε κατασκευή προσφορών εκτελούνται δύο διορθώσεις πιστότητας του στόλου. Η \emph{συμπλήρωση στόλου} (fleet completion) προσθέτει, ανά τεχνολογία, τη συγκεντρωτική ικανότητα που λείπει από τον κατάλογο μονάδων --- σε ζώνες με ελλιπή δεδομένα ανά μονάδα, ο στόλος του καταλόγου υπολείπεται συστηματικά του πραγματικού. Η \emph{απομείωση στην πραγματική διαθεσιμότητα} (fleet truthing) περιορίζει, αντίστροφα, τις τεχνολογίες βάσης στο πρόσφατα παρατηρημένο μέγιστό τους (95ο εκατοστημόριο κυλιόμενου 30ημέρου, με περιθώριο), όταν ο «χάρτινος» στόλος υπερβαίνει ό,τι πραγματικά λειτουργεί --- η διόρθωση που εξάλειψε το λιγνιτικό «φάντασμα» του 2022 (Ενότητα~\ref{subsec:crisis-2022}). Και οι δύο χρησιμοποιούν αποκλειστικά ιστορική πληροφορία διαθέσιμη πριν από τη δημοπρασία.

\subsection{Από τον Σκελετό στις Περιφερειακές Στρατηγικές}
\label{subsec:skeleton-to-regions}

Διαβάζοντας τις περιφερειακές στρατηγικές του ευρωπαϊκού αποτυπώματος (Ενότητα~\ref{subsec:region-strategies}) πάνω σε αυτόν τον σκελετό, αναδεικνύεται ότι πρόκειται για τρεις μόνο οικογένειες αποκλίσεων από ένα κοινό σχήμα: (1)~\emph{αλήθεια κόστους} --- διόρθωση του ίδιου του SRMC (ο πολλαπλασιαστής της Ιταλίας, το πυρηνικό πάτωμα της Γαλλίας)· (2)~\emph{ανατιμολόγηση σε κόστος ευκαιρίας} του κυρίαρχου ευέλικτου πόρου έναντι της συζευγμένης τιμής του πρώτου περάσματος (οι υδροηλεκτρικές άγκυρες, η πυρηνική άγκυρα, η τιμολόγηση των βελγικών εισαγωγών --- όλα ο ίδιος μηχανισμός)· και (3)~\emph{ιδιοσυγκρασία στενότητας} --- πόσο επιθετικά προσαυξάνονται οι ανώτερες βαθμίδες, δηλαδή μια δήλωση για το πόσο συχνά κάθε περιοχή είναι πραγματικά στενή.

\subsection{Στρατηγικές Προσφορών ανά Περιοχή: τα Προφίλ Ζωνών (\texttt{ZoneProfile})}
\label{subsec:zone-profiles}

Η πλήρης παραμετροποίηση των περιφερειακών στρατηγικών υλοποιείται ως ένα προφίλ ανά ζώνη (δομή \texttt{ZoneProfile} στο \texttt{src/MeritOrderBook.jl}): οι στρατηγικές είναι \emph{δεδομένα}, όχι νέος κώδικας, και κάθε επιλογή αιτιολογείται από τη φυσική της περιοχής ή από δημόσια δεδομένα --- ποτέ από προσαρμογή στις τιμές έναντι των οποίων βαθμολογείται το μοντέλο. Η κοινή μηχανή, όπως περιγράφηκε στις προηγούμενες υποενότητες, είναι παντού η ίδια: προσφορές θερμικών μονάδων στο SRMC (TTF/βαθμός απόδοσης $+$ EUA $\times$ συντελεστής εκπομπών $+$ O\&M, \textbf{χωρίς κανένα προσαρμοσμένο περιθώριο κέρδους}), διαχωρισμός must-run του τεχνικού ελαχίστου των δεσμευμένων μονάδων, ΑΠΕ ως δέκτες τιμής στο 1~€/MWh, σχεδόν ανελαστική ζήτηση ($\approx$98\% στην οροφή των 3.000~€/MWh με 2\% ελαστική ουρά), πίστωση της διαθέσιμης ικανότητας \emph{εισαγωγών} στο περιθώριο στενότητας όπου μια ζώνη στηρίζεται δομικά σε εισαγωγές, και ο όρος στενότητας της Εξίσωσης~\ref{eq:scarcity-factor} όταν το περιθώριο πραγματικά στενεύει.
...
\begin{itemize}
\item \textbf{Υδροηλεκτρικές ζώνες (Νορβηγία, αλπικές CH/AT, ΝΑ Ευρώπη):} τα υδροηλεκτρικά ταμιευτήρα δεν προσφέρουν στο (σχεδόν μηδενικό) μεταβλητό τους κόστος αλλά στο \emph{κόστος ευκαιρίας} τους --- μια αξία νερού συνδεδεμένη με την τιμή του αερίου, μειωμένη όταν οι ταμιευτήρες είναι πλήρεις και αυξημένη σε ξηρασία (εβδομαδιαία πληρότητα ταμιευτήρων έναντι της εποχικής νόρμας)· ο σουηδικός βορράς (SE1/SE2) ακολουθεί επιπλέον το εποχικό σήμα εκφόρτισης (seasonal drawdown) του ταμιευτήρα.
\item \textbf{Άγκυρες κόστους ευκαιρίας δύο περασμάτων:} στις υδρο-αγκυρωμένες ζώνες (Νορβηγία, CH, AT, BE, SK) και στη πυρηνικά αγκυρωμένη Γαλλία, ο ευέλικτος στόλος επανα-προσφέρει σε δεύτερο πέρασμα έναντι της \emph{συζευγμένης} τιμής του πρώτου περάσματος: ένας νορβηγικός ταμιευτήρας ή ένα γαλλικό πυρηνικό ρωτά «πόσο αξίζει η ενέργειά μου για ολόκληρη τη συνδεδεμένη αγορά;», όχι μόνο τοπικά. Όλα τα σημεία αναφοράς είναι εσωτερικά του μοντέλου --- η ιδιότητα no-fit/ex-ante διατηρείται.
\item \textbf{Ηπειρωτικές θερμικές ζώνες (DE\_LU, NL, PL, CZ) και Βαλτικές (EE, LT, LV):} ο στόλος αληθεύεται στην \emph{εγκατεστημένη} ικανότητα του μητρώου (\texttt{fleet\_truth\_mode=:installed}), διότι το ιστορικό παραγωγής ανά μονάδα του ENTSO-E υποεκτιμά συστηματικά τους στόλους αυτούς· οι Βαλτικές πιστώνουν επιπλέον τις διασυνδέσεις εισαγωγών τους (Estlink, NordBalt) στο περιθώριο στενότητας, αφού οι ισχνοί εγχώριοι στόλοι τους «ταξιδεύουν» πάνω στο σκανδιναβικό σύστημα.
\item \textbf{Ιταλία (επτά υποζώνες):} ειδικό προφίλ για τη δομικά στενότερη, καθοριζόμενη από το αέριο αγορά της χερσονήσου, με δικές της παραμέτρους στενότητας.
\item \textbf{Εντιμότητα κρίσης (απομείωση p95):} όπου η πραγματική διαθεσιμότητα μιας τεχνολογίας υπολείπεται πολύ της ονομαστικής (π.χ. φθίνοντες λιγνιτικοί στόλοι), η ικανότητα απομειώνεται στο πρόσφατο 95ο εκατοστημόριο παραγωγής της (Ενότητα~\ref{subsec:fleet-honesty}), ώστε το βιβλίο να αντικατοπτρίζει τις μονάδες που όντως λειτουργούν.
\end{itemize}

Τα προφίλ τεκμηριώνονται ζώνη προς ζώνη στον άτλαντα βαθμονόμησης του αποθετηρίου (\texttt{docs/calibration-atlas.md})· τα ποσοτικά τους αποτελέσματα στην ετήσια αξιολόγηση παρουσιάζονται ανά καθεστώς στρατηγικής στην Ενότητα~\ref{sec:cv16-fullyear}.

\subsection{Συγχώνευση Εντολών}
\label{subsec:order-merging}

Εντολές ίδιας τεχνολογίας, στρογγυλοποιημένης τιμής και χρονικού διαστήματος συγχωνεύονται πριν την εκκαθάριση, μειώνοντας το μέγεθος του βιβλίου κατά $\approx 6\times$ (τυπικά από $\sim$11.000 σε $\sim$1.700 εντολές ανά ζώνη-ημέρα) χωρίς καμία επίδραση στις τιμές εκκαθάρισης. Η συγχώνευση είναι καθοριστική για την πολυζωνική εκκαθάριση, όπου κάθε εντολή εισάγει δυαδικές μεταβλητές συμπληρωματικότητας στο πρόβλημα μικτού ακέραιου προγραμματισμού.
...
Δύο λεπτομέρειες της υλοποίησης αποδείχθηκαν καθοριστικές για τον ορθό προσδιορισμό των τιμών. Πρώτον, απαιτούνται \emph{αμφότερες} οι πλευρές της συμπληρωματικότητας: πέραν της παραπάνω (αποδοχή $\perp$ dual υπόλοιπο), προστίθεται και η συμμετρική συνθήκη dual $\perp$ $(1 - \text{αποδοχή})$, ώστε μόνο πλήρως αποδεκτές εντολές να φέρουν θετικό πλεόνασμα. Χωρίς τη δεύτερη πλευρά, μια μερικώς αποδεκτή (οριακή) εντολή δεν καθηλώνει την τιμή στην προσφορά της και το πρόβλημα προσδιορίζει την τιμή μόνο εντός διαστήματος, με τον επιλυτή να επιστρέφει αυθαίρετο σημείο του. Δεύτερον, οι σταθερές Big-M ορίζονται \emph{ανά εντολή} από το μέγιστο δυνατό πλεόνασμά της (ποσότητα επί απόσταση προσφοράς από τα όρια τιμών)· μια καθολική σταθερά είτε στρεβλώνει τις τιμές μεγάλων εντολών είτε καθιστά το πρόβλημα αδύνατο για προσφορές εκτός των ορίων του βιβλίου. Τέλος, επειδή η αντικειμενική συνάρτηση δεν περιλαμβάνει τις μεταβλητές τιμών, η τελική τιμή κάθε ζώνης-διαστήματος ανακατασκευάζεται ντετερμινιστικά από το μοτίβο αποδοχής (η οριακή εντολή ορίζει την τιμή· ελλείψει οριακής, επιλέγεται το ανταγωνιστικό άκρο του εφικτού διαστήματος), με ομαδοποίηση των ζωνών σε συνιστώσες ίσης τιμής όταν οι διασυνδέσεις δεν είναι συμφορημένες και επικύρωση του προσήμου των μισθωμάτων συμφόρησης.
...
if has_network_constraints
    ...
    # -S + D + shed - εισροές + εκροές = 0, δηλ. S + εισροές = D + shed + εκροές
    @constraint(model, [node_id in nodes, period in periods],
        ...
        load_shed[node_id, period] -
        # Εισροές από άλλες ζώνες (μειώνουν την απαιτούμενη τοπική παραγωγή)
        sum(flow[(z, node_id), period] for z in connected_zones_to[node_id]) +
        # Εκροές προς άλλες ζώνες (αυξάνουν την απαιτούμενη τοπική παραγωγή)
        sum(flow[(node_id, z), period] for z in connected_zones_from[node_id]) == 0
    )
else
...
\subsection{Όρια Βάσει Ροών από τα Δεδομένα JAO}
\label{subsec:jao-network}

Η πιο πρόσφατη εξέλιξη του εμπλουτισμένου δικτύου (μοντέλο cv35, Αύγουστος 2026) αντικαθιστά τα αλλοιωμένα κατάλοιπα ATC των flow-based περιοχών με τα ίδια τα όρια του υπολογισμού ικανότητας της αγοράς (Ενότητα~\ref{subsec:jao-data}). Τρεις μηχανισμοί, καθένας με δικό του διακόπτη αδρανοποίησης:

\begin{enumerate}
\item \textbf{Το maxBEX ως ATC} (\texttt{Network.jao\_maxbex}): κάθε ωριαίο κελί συνόρου \emph{χωρίς} εγγραφή Day-ahead στον πίνακα ENTSO-E παίρνει τη μέγιστη διμερή ανταλλαγή του JAO· σύνορα που το JAO δημοσιεύει και ο πίνακας ENTSO-E δεν φέρει καθόλου \emph{προστίθενται}· και τα σύνορα Core που η βαθμονόμηση των cv15/cv17 είχε \emph{αποσύρει} λόγω των πλασματικών υπολειμμάτων (AT--CZ, AT--DE\_LU, AT--SI, SK--CZ, SK--PL, \ldots) ξαναγίνονται ενδογενή --- η απόσυρση ήταν θεραπεία της κακής πηγής, όχι της φυσικής, και με σωστή πηγή αίρεται μαζί με την αντίστοιχη έγχυση παρατηρούμενων εισαγωγών.
\item \textbf{Κλιμάκωση στην καθαρή θέση κόμβου} (\texttt{\_scale\_maxbex\_by\_net\_position}): κάθε maxBEX είναι το μέγιστο \emph{ενός} συνόρου με όλα τα άλλα στο μηδέν --- τα διμερή μέγιστα δεν ισχύουν ταυτόχρονα. Χρησιμοποιούμενα ως ανεξάρτητα όρια αφήνουν έναν κόμβο να εξάγει σε όλα τα σύνορα μαζί το μέγιστό τους (μετρημένο στη Γαλλία: μοντελοποιημένη καθαρή εξαγωγή 16,4~GW έναντι πραγματικού μέσου $\sim$10~GW, με άθροισμα διμερών μεγίστων 34~GW). Γι' αυτό, ανά κόμβο, περιοχή και ώρα, αν το άθροισμα των εξερχόμενων ικανοτήτων JAO υπερβαίνει τη μέγιστη καθαρή θέση του κόμβου, κλιμακώνονται αναλογικά προς τα κάτω (ομοίως οι εισερχόμενες προς την ελάχιστη). Η ακριβής εκδοχή --- ο περιορισμός καθαρής θέσης \emph{μέσα} στο MPCC με δικό του δυϊκό όρο στη συνθήκη σύζευξης τιμών --- κατασκευάστηκε, μετρήθηκε ασύμβατη με τη διμερή συμπληρωματικότητα ως έχει, και παραμένει καταγεγραμμένη προαιρετική κατεύθυνση για επόμενη εργασία στον πυρήνα του επιλυτή.
\item \textbf{Πλαφόν από μη διαθεσιμότητες δικτύου} (\texttt{Network.tx\_outage\_caps}): ανά κατευθυνόμενο σύνορο-ώρα, το ελάχιστο της εναπομένουσας NTC των ενεργών μηνυμάτων μη διαθεσιμότητας πλαφονάρει την ικανότητα από οποιαδήποτε πηγή (Day-ahead ATC, JAO, αποδεδειγμένη ικανότητα), με τον ίδιο κανόνα έκδοσης-πύλης που εφαρμόζεται στις μη διαθεσιμότητες μονάδων.
\end{enumerate}

Μία πρακτική σημείωση επιλυτή: στο πλήρες δίκτυο των 108 ενδογενών συνδέσμων ο HiGHS εμφάνισε σφάλματα κατάτμησης σε $\sim$40\% των ημερών του πειράματος αποδοχής· οι εκτελέσεις του δικτύου JAO δρομολογούνται στον Gurobi (με τον HiGHS να παραμένει ο προεπιλεγμένος επιλυτής των υπόλοιπων διαδρομών).
...
% =============================================================================
\section{Υποδομή Αναπαραγωγιμότητας και Σεναρίων}
\label{sec:scenario-infra}

Δύο όψιμες προσθήκες μετατρέπουν το σύστημα από αγωγό παραγωγής τιμών σε \emph{ερευνητικό όργανο} που τρίτοι μπορούν να αναπαραγάγουν και να ανακρίνουν.

\subsection{Αυτοτελές Απόσπασμα Δεδομένων (DuckDB)}
\label{subsec:duckdb-extract}

Η βιβλιοθήκη μπορεί να διαβάζει, αντί της ζωντανής PostgreSQL, ένα \textbf{αυτοτελές απόσπασμα DuckDB} --- ένα μόνο αρχείο που καθρεφτίζει τα ίδια ονόματα σχημάτων και πινάκων, ώστε τόσο η μονοζωνική τιμολόγηση όσο και η πλήρης πολυζωνική εκκαθάριση των 39 ζωνών να εκτελούνται εντελώς εκτός σύνδεσης. Το στρώμα πρόσβασης δεδομένων κάνει διεκπεραίωση (dispatch) ανά υποστήριγμα με μικρή μεταγραφή διαλέκτου SQL· τα αποτελέσματα των εκτελέσεων γράφονται σε χωριστό αρχείο αποτελεσμάτων ώστε τα πηγαία δεδομένα να μένουν μόνο για ανάγνωση. Η ισοδυναμία με την PostgreSQL είναι μετρημένη: οι μονοζωνικές τιμές είναι bit-πανομοιότυπες, ενώ στην πολυζωνική εκκαθάριση των 39 ζωνών $\sim$98\% των τιμών είναι bit-πανομοιότυπες και οι υπόλοιπες συμφωνούν σε $\le 2 \times 10^{-12}$~€/MWh (μη ντετερμινισμός τελευταίου ψηφίου στη σειρά άθροισης των συναρτήσεων συγκεντρωτικών αποτελεσμάτων SQL). Ένα δημοσιευμένο απόσπασμα 39 ζωνών (2023--2026, μορφή Parquet με κατακερματισμούς SHA-256) επιτρέπει σε οποιονδήποτε να αναπαραγάγει το αντιπαράδειγμα χωρίς καμία πρόσβαση σε βάση --- η μεθοδολογία είναι ελέγξιμη από άκρη σε άκρη.

\subsection{Το API Σεναρίων}
\label{subsec:scenario-api}

Πέντε προαιρετικά σημεία αγκίστρωσης (hooks) στην κατασκευή του βιβλίου εντολών καθιστούν τα αντιπαραδείγματα πολιτικής πρώτης τάξεως λειτουργία:

\begin{itemize}
\item \texttt{load\_modifier} / \texttt{renewable\_modifier}: μετασχηματίζουν τη χρονοσειρά φορτίου ή ΑΠΕ \emph{στην πηγή}, ώστε η αλλαγή να διαδοθεί σε ό,τι παράγεται από αυτές (καθαρή ζήτηση, περιθώριο στενότητας, αξίες νερού).
\item \texttt{extra\_orders}: προσθέτει εντολές προσφοράς ή ζήτησης (π.χ. νέος καταναλωτής που ζητά ισχύ στο όριο τιμής).
\item \texttt{strategist}: λαμβάνει το πλήρες βιβλίο με ετικέτα ιδιοκτήτη ανά εντολή και χάρτη μονάδας$\to$εταιρείας, και το ξαναγράφει --- π.χ. «τι θα γινόταν αν η δεσπόζουσα εταιρεία υπερτιμολογούσε τις μονάδες της 20\%;».
\item \texttt{fleet\_modifier}: προσθέτει, αφαιρεί ή απομειώνει φυσικές μονάδες ως δεδομένα (απόσυρση, νέα μονάδα), μετά τις διορθώσεις πιστότητας στόλου ώστε η αλλαγή να μην «αυτοθεραπεύεται».
\end{itemize}

Στην πολυζωνική διαδρομή τα πέντε σημεία δεσμοποιούνται σε δομή \texttt{ZoneScenario}, εφαρμόσιμη ενιαία ή ανά ζώνη (\texttt{Dict\{String,ZoneScenario\}}), και διατρέχουν και τα δύο περάσματα της εκκαθάρισης --- άρα ένα σοκ σε μία ζώνη μεταδίδεται μέσω των αναφορών άγκυρας στα κόστη ευκαιρίας όλων των αγκυρωμένων ζωνών. Όταν κανένα σημείο δεν ορίζεται, η διαδρομή είναι \emph{δυαδικά πανομοιότυπη} (byte-identical) με τη μη τροποποιημένη (ελεγμένο σε πλήρη βιβλία), οπότε το API δεν επιβαρύνει το παραγωγικό προϊόν. Κάθε εκτέλεση σεναρίου φέρει διακριτή ετικέτα \texttt{clearing\_mode} στη βάση αποτελεσμάτων, και ένα επικυρωμένο ερώτημα (\texttt{queries/load\_weighted\_price\_delta.sql}, με οδηγό \texttt{bin/scenario\_delta.jl}) υπολογίζει τη σταθμισμένη στο φορτίο μεταβολή τιμής και το πρόσθετο κόστος καταναλωτών μεταξύ δύο ετικετών. Οι μελέτες περίπτωσης που εκτελέστηκαν με την υποδομή αυτή παρουσιάζονται στην Ενότητα~\ref{sec:scenario-studies}.
