...
struct Generator
    ...
    # Παράμετροι από εξαγωγή ιστορικών δεδομένων
    ramp_up::Float64       # Ρυθμός αύξησης (κλάσμα p_max/ώρα)
    ramp_down::Float64     # Ρυθμός μείωσης (κλάσμα p_max/ώρα)
...
Ο τύπος καυσίμου (\texttt{fuel\_type}) αποθηκεύεται ως \texttt{Symbol} για αποτελεσματική σύγκριση και χρησιμοποιείται για την ανάκτηση παραμέτρων ειδικών ανά τύπο καυσίμου. Τα πεδία \texttt{ramp\_up}, \texttt{ramp\_down}, \texttt{min\_uptime} και \texttt{min\_downtime} αποθηκεύουν τεχνικές παραμέτρους που εξάγονται αυτόματα από ιστορικά δεδομένα παραγωγής, όπως περιγράφεται στην Ενότητα~\ref{subsec:parameter-inference}.
...
\begin{samepage}
\begin{minted}[fontsize=\small,linenos,frame=lines]{julia}
function infer_ramp_rates(generation_data::Vector{Float64}, p_max::Float64;
                          resolution_minutes::Int=60)
    # Υπολογισμός μεταβολών μεταξύ διαδοχικών μετρήσεων
    deltas = diff(generation_data)

    # Διαχωρισμός σε αυξήσεις και μειώσεις
    ramp_up_values = filter(x -> x > 0, deltas)
    ramp_down_values = filter(x -> x < 0, deltas) .|> abs

    # 95ο εκατοστημόριο (αγνόηση ακραίων τιμών)
    ramp_up_mw = length(ramp_up_values) >= 100 ?
        quantile(ramp_up_values, 0.95) : nothing
    ramp_down_mw = length(ramp_down_values) >= 100 ?
        quantile(ramp_down_values, 0.95) : nothing

    # Κανονικοποίηση σε κλάσμα της p_max ανά ώρα
    hours_per_period = resolution_minutes / 60
    ramp_up = ramp_up_mw !== nothing ?
        (ramp_up_mw / p_max) / hours_per_period : nothing
    ramp_down = ramp_down_mw !== nothing ?
        (ramp_down_mw / p_max) / hours_per_period : nothing
...
\begin{samepage}
\begin{minted}[fontsize=\small,linenos,frame=lines]{julia}
function infer_p_min(generation_data::Vector{Float64}, p_max::Float64;
                     fuel_type::Symbol=:unknown)
    # Φιλτράρισμα μηδενικών τιμών (μονάδα εκτός λειτουργίας)
    non_zero = filter(x -> x > 0, generation_data)

    # Ανίχνευση μεταβατικών καταστάσεων (|Δ| > 5% της p_max)
    deltas = diff(generation_data)
    transient_threshold = 0.05 * p_max
    stable_indices = findall(i -> abs(deltas[i]) <= transient_threshold,
                             1:length(deltas))

    stable_values = generation_data[stable_indices .+ 1]
    stable_non_zero = filter(x -> x > 0, stable_values)

    if length(stable_non_zero) < 50
        return nothing  # Ανεπαρκή δεδομένα
    end

    # 5ο εκατοστημόριο της σταθερής λειτουργίας
    p_min_raw = quantile(stable_non_zero, 0.05) / p_max

    # Εφαρμογή ορίων ανά τύπο καυσίμου
    bounds = get_p_min_bounds(fuel_type)
    return clamp(p_min_raw, bounds.min, bounds.max)
end
\end{minted}
\end{samepage}
...
Οι ευέλικτοι πόροι (υδροηλεκτρικά, μπαταρίες) λαμβάνουν $P^{\min} = 0$ χωρίς εξαγωγή, καθώς μπορούν να λειτουργήσουν σε οποιοδήποτε επίπεδο ισχύος.
...
\begin{samepage}
\begin{minted}[fontsize=\small,linenos,frame=lines]{julia}
function infer_uptime_downtime(generation_data::Vector{Float64};
                               resolution_minutes::Int=60)
    # Προσδιορισμός κατάστασης (on: > 1 MW, off: = 0)
    on_off = generation_data .> 1.0

    # Εντοπισμός κύκλων λειτουργίας
    uptime_durations = Float64[]
    downtime_durations = Float64[]
    current_duration = 0

    for i in 2:length(on_off)
        if on_off[i] == on_off[i-1]
            current_duration += 1
        else
            # Τέλος κύκλου
            hours = current_duration * resolution_minutes / 60
            if on_off[i-1]  # Ήταν on
                push!(uptime_durations, hours)
            else            # Ήταν off
                push!(downtime_durations, hours)
            end
            current_duration = 1
        end
    end

    # 5ο εκατοστημόριο (φιλτράρισμα σύντομων διακοπών/glitches)
    min_uptime = length(uptime_durations) >= 10 ?
        quantile(uptime_durations, 0.05) : nothing
    min_downtime = length(downtime_durations) >= 10 ?
        quantile(downtime_durations, 0.05) : nothing

    return (min_uptime=min_uptime, min_downtime=min_downtime)
end
\end{minted}
\end{samepage}
...
# Ελάχιστος χρόνος λειτουργίας: προτίμηση σε εξαχθείσες τιμές
for (i, gen) in enumerate(generators)
    UT = gen.min_uptime > 0 ? gen.min_uptime : fuel_params[i].min_uptime
    if UT > 1
...
# Ελάχιστος χρόνος αδράνειας: προτίμηση σε εξαχθείσες τιμές
for (i, gen) in enumerate(generators)
    DT = gen.min_downtime > 0 ? gen.min_downtime : fuel_params[i].min_downtime
    if DT > 1
...
\begin{samepage}
\begin{minted}[fontsize=\small,linenos,frame=lines]{julia}
# Υπολογισμός ρυθμών μεταβολής: προτίμηση σε εξαχθείσες τιμές
# με fallback σε παραμέτρους τύπου καυσίμου
ramp_up = [gen.ramp_up > 0 ? gen.ramp_up * gen.p_max * period_hours :
           fuel_params[i].ramp_up_rate * gen.p_max * period_hours
           for (i, gen) in enumerate(generators)]
ramp_down = [gen.ramp_down > 0 ? gen.ramp_down * gen.p_max * period_hours :
             fuel_params[i].ramp_down_rate * gen.p_max * period_hours
             for (i, gen) in enumerate(generators)]
