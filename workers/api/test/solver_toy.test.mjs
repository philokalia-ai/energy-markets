/* Solver view (pillar 1) — unit + DOM smoke test, no jsdom, no network.
 *
 ...
 *
 * Part B: a DOM smoke of the Solver view against the bundled fixtures — drives
 *   renderSolverToy / renderSolverDual / renderSolverPass and the live-data S4
 *   exemplar (fetching web/fixtures/flows + zones through a disk-backed fetch
 *   stub) and asserts the rendered SVG + the client-computed separation/rent,
 *   plus the graceful empty state on a flows-absent exemplar.
 *
 * Runs fully offline (?live=0 + a disk fetch stub). No exchange bids anywhere.
 */
...
const fixturesDir = join(repo, "web/fixtures");
...
// ?live=0 → the app skips the network rung and reads ./fixtures directly.
globalThis.window = { location: { search: "?live=0" }, __EUPHEMIA_NO_AUTOINIT: true };
globalThis.document = {
...
// disk-backed fetch stub: "./fixtures/flows/<date>" / "./fixtures/zones/<Z>.json".
globalThis.fetch = (path) => {
  const rel = String(path).replace(/^\.\/fixtures\//, "");
  const file = join(fixturesDir, rel);
  if (existsSync(file)) {
...
near(r0.pN, 20, 1e-6, "ATC 0: NORTH prices its cheap block");
near(r0.pS, 95, 1e-6, "ATC 0: SOUTH prices its dear block");
...
near(r8.pN, 70, 1e-6, "ATC 800: NORTH now on its dear block");
near(r8.pS, 95, 1e-6, "ATC 800: SOUTH still dear");
...
// the load-bearing invariant across the whole slider range
let invariantOK = true, couplingSeen = false;
for (let atc = 0; atc <= 2500; atc += 50) {
  const r = clear(atc);
  if (r.congested) { if (Math.abs(r.flow - atc) > 1e-6) invariantOK = false; }   // congested ⇔ flow = ATC
  else { couplingSeen = true; if (r.rent !== 0 || Math.abs(r.gap) > 1e-6) invariantOK = false; }  // coupled ⇔ gap 0, rent 0
...
/* ========================= Part B — DOM smoke ========================= */
// S3 toy: drive the slider render at a congested and a coupled setting.
...
S.renderSolver();   // renders dual, toy, pass, exemplar steps, exemplar
...
// S4 exemplar 0 (FR·IT-NORTH, 18:00) — congested, computed live from fixtures.
await new Promise((r) => setTimeout(r, 40));
const ex = getEl("solver-exemplar");
ok(ex.querySelector("svg") != null, "S4: exemplar 0 renders the two-tile SVG from fixtures");
...
// S4 exemplar with no fixture (DE_LU·FR 2026-06-21) — graceful empty state.
getEl("solver-exemplar-steps").children[2]._fire("click");
await new Promise((r) => setTimeout(r, 40));
ok(/No persisted coupled flow/.test(getEl("solver-exemplar").textContent),
   "S4: a flows-absent exemplar degrades to the empty state (no crash)");
