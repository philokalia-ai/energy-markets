...
async function buildPayload(env, route, zone, date) {
  ...
  if (route === "book_methodology") {
    // The pillar-5 bid-methodology object (cost model, form constants, strategy
    // glossary, provenance, cv-ledger) — generated by bin/export_book_methodology.jl
    // and published as a JSON pass-through, like the manifest (no parquet decode).
    // 404s until it is explicitly published; there is NO fixture fallback — the
    // SPA shows the shared honest "live data unavailable" state (fix/no-runtime-fixtures).
    const obj = await env.DATA.get("v1/book_methodology.json");
    return obj ? await obj.text() : null;
  }
  if (route === "zone_strategies") {
    // The resolved per-zone ZoneProfile calibration table (bin/export_zone_strategies.jl),
    // consumed by the About page AND the methodology zone-profile explorer (B5).
    // JSON pass-through; 404s until explicitly published (no snapshot fallback —
    // the SPA/About surface an honest empty state, never synthetic data).
...
export default {
  async fetch(request, env, ctx) {
    ...
    if ((m = url.pathname.match(/^\/api\/v1\/zones\/([A-Za-z0-9_-]+)$/))) {
      ...
      route = "manifest";
    } else if (url.pathname === "/api/v1/book_methodology" || url.pathname === "/api/v1/book_methodology.json") {
      // JSON pass-throughs, matched with or without the .json suffix: the SPA's
      // apiPath() strips .json, but the About page requests the literal file.
