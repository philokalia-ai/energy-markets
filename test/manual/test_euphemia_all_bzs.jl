...
"""
    test_euphemia_with_retry(zone, date; max_retries=2, retry_delay=5.0, order_method=:uc_based, model=:mpcc, optimizer="highs", save_to_db=true)
...
"""
    test_euphemia_market_clearing(zone, date; order_method=:uc_based, model=:mpcc, optimizer="highs", markup_factor=1.1, random_seed=nothing, save_to_db=true)
...
function test_euphemia_market_clearing(zone, date;
    ...
    if save_to_db
        try
            ...
            saved_records = save_energy_prices(prices, zone, date, order_method)
            println("   ✅ Successfully saved $saved_records records to database")
        catch e
            @warn "Failed to save prices to database for zone $zone: $e"
            # Don't fail the test if database save fails, just warn
        end
    end
    ...
    for config in test_configs
        ...
        for zone in zones
            ...
            try
                ...
                println("   💾 Database: $(result.saved_records) records saved")
        ...
        if !isempty(successful_zones)
            ...
            for zone in successful_zones
                ...
                total_saved_records += result.saved_records
            end
            ...
            if !isempty(all_prices)
                ...
                println("💾 Total database records saved: $total_saved_records")
