...
println("Testing Euphemia for bidding zone: $bidding_zone and date: $date")

# Test individual components
println("=== Testing Generators ===")
generators::Vector{Generator} = get_generators(date)
...
println("\n=== Testing Loads ===")
loads::Vector{Load} = get_loads(bidding_zone, date)
...
renewables_generation::Vector{RenewablesGenerationForecast} =
    get_generation_forecast_for_wind_and_solar(bidding_zone, date)
