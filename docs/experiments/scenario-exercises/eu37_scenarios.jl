...
#
#   eu37_dc574          : GR data center +574 MW (load_modifier),
#                         2024-07-01..2026-06-30 (730 d)
#   eu37_ops_floor_paneu: pan-EU cold ironing, FLOOR profile, per-zone
#                         extra_orders (24 zones), 2024-07-01..2025-06-30
#                         (365 d — the AIS data window's overlap with the DC one)
#
#   julia --project=. docs/experiments/scenario-exercises/eu37_scenarios.jl
#
...
let guard_day = Date(2025, 4, 15)
    ...
    stored = Euphemia.sql2df("""
        ...
        WHERE clearing_mode = 'multi_zone_eu' AND code_version = 37
          AND date_time_utc >= \$1::date AND date_time_utc < \$1::date + 1""",
        [guard_day])
    nrow(stored) >= 900 || (println("GUARD_FAIL stored rows=$(nrow(stored))"); println("WARN: record-pairing guard failed — using the fresh eu37_base arm instead (cv37 run-to-run nondeterminism, see ledger note)"))
    ...
    (maxd <= 1e-9 && nmiss == 0 && ncmp >= 900) ||
        (println("GUARD_FAIL maxdelta=$maxd missing=$nmiss"); println("WARN: guard mismatch tolerated — fresh eu37_base arm is the pairing baseline"))
...
RUNS = [
    ("eu37_base",            nothing,         Date(2024, 7, 1), Date(2026, 6, 30)),
    ("eu37_dc574",           dc_scenario,     Date(2024, 7, 1), Date(2026, 6, 30)),
    ("eu37_ops_floor_paneu", pan_ops_scenario, Date(2024, 7, 1), Date(2025, 6, 30)),
]
for (label, scn, d0, d1) in RUNS
    ...
    r = run_pipelined_backfill(collect(days), FOOTPRINT;
        solver_workers=4, book_workers=10, optimizer="gurobi",
