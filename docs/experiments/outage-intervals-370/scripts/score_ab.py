...
sim = pd.read_sql("""select clearing_mode arm, bidding_zone z, (date_time_utc at time zone 'UTC') t, price_eur_mwh p
...
# paired days = days where BOTH arms have all 39 zones x 24 h
cnt = wide.groupby("d").size(); full = cnt[cnt == cnt.max()].index
wide = wide[wide.d.isin(full)]
m = wide.merge(act, on=["z", "t"]).merge(load, on=["z", "t"], how="left")
print(f"paired days: {len(full)}  zones: {m.z.nunique()}  cells: {len(m)}  (days with every zone-hour in both arms)")
def score(g, arm):
    e = g[arm] - g.a
