#!/usr/bin/env julia
# Score the cv24 A/B (ab_cv24.jl outputs) against REALIZED day-ahead prices read
...
# cv24, plus the deltas. Prints the gate-zone table and the footprint means.
#
#   EUPHEMIA_DUCKDB_PATH=.../x.duckdb julia --project=. \
#     docs/experiments/cv24/score_cv24.jl base.tsv cv24.tsv windows.json
using DuckDB, Printf, DataFrames, CSV, Statistics, Dates, JSON
basef, cv24f, winf = ARGS[1], ARGS[2], ARGS[3]
...
b = loadarm(basef); c = loadarm(cv24f)
rename!(b,:price=>:base); rename!(c,:price=>:cv24)
...
GATE = ["IT-NORTH","IT-CNORTH","IT-CSOUTH","IT-SOUTH","IT-Calabria","IT-Sicily","IT-Sardinia","CH","SI","AT","FR","GR"]
for (wname, wdays) in sort(collect(windows); by=first)  # by key: Set values are not comparable
    ...
    @printf("%-6s | %-28s | %-28s | %s\n","zone","base MAE/bias/corr eveB/eveMAE","cv24 MAE/bias/corr eveB/eveMAE","Δ")
    for z in GATE
        g = w[w.zone.==z,:]
        isempty(g) && continue
        bm,bb,bc,bem,beb = score(g,:base)
        cm,cb,cc,cem,ceb = score(g,:cv24)
    ...
    for z in unique(w.zone)
        g=w[w.zone.==z,:]
        bm,_,bc,_,beb=score(g,:base); cm,_,cc,_,ceb=score(g,:cv24)
