...
S = sys.argv[1]; BOOKS = 'data/web/v1/books'
wed = [dt.date(2025,7,2)+dt.timedelta(days=7*i) for i in range(52)]
days = [d for d in wed if os.path.exists(f'{BOOKS}/{d}.parquet')]
print(f"days with books: {len(days)}/52 (missing: {[str(d) for d in wed if d not in days]})")
con = duckdb.connect()
# --- flows -> actual net export per zone-hour (UTC hour key)
fl = con.sql(f"""select ts, o, i, mw from read_csv('{S}/probe_flows.csv', header=false,
...
jn = pd.read_csv(f'{S}/probe_jaonp.csv', header=None, names=['hub','ts','mn','mx'])
...
for day in days:
    ...
    for (z, k), g in b.groupby(['zone','k']):
        ...
        nxr = nx[(nx.zone==z)&(nx.k==k)]
        nxv = float(nxr.nx.iloc[0]) if len(nxr) else np.nan
...
def loadcsv(f):
    ...
    d['k'] = d.ts.str[:13]; return d[['zone','k','p']]
st_ = loadcsv(f'{S}/eval_settled.csv').rename(columns={'p':'settled'})
sim = loadcsv(f'{S}/arm_ab_jao_np.csv').rename(columns={'p':'sim35'})
...
df.to_parquet(f'{S}/probe_dataset.parquet')
