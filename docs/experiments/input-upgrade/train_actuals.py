...
OUTDIR=f"{SP}/models39_actuals"; os.makedirs(OUTDIR, exist_ok=True)
S0=F.S0
...
TGT_ACT =pd.read_parquet(f"{SP}/tgt_actuals.parquet"); TGT_ACT["h"]=pd.to_datetime(TGT_ACT["h"])
...
def fit_score(z,target,df,feat_cols,tgt_col,baseline_pred,clamp_night=False,ref_col=None,da_col=None):
    ...
    if da_col is not None:
        mfc=metrics(va[da_col].values,va[tgt_col].values)
        scorecard.append(dict(zone=z,target=target,model="tso_fc",win=False,**mfc))
        print(f"  {z}_{target}: TSO-fc-vs-actuals mae={mfc['mae']:.1f} corr={mfc['corr']:.3f}")
...
for z in ZONES:
    ...
    ta=TGT_ACT[TGT_ACT.zone==z][["h","solar_act","wind_act"]]; ra=ra.merge(ta,on="h",how="left")
    ...
    if GEOM[z]["solar"]:
        ...
        fit_score(z,"solar",ra,feat_s,"solar_act",bs,clamp_night=True,ref_col="cap95_solar",da_col="solar_da")
    else:
    ...
    if GEOM[z]["wind"]:
        ...
        fit_score(z,"wind",ra,feat_w,"wind_act",bw,ref_col="cap95_wind",da_col="wind_da")
    else:
        print(f"  {z}_wind: no meaningful wind (geom) -> pack/zero passthrough")
    # LOAD skipped in the actuals-target variant

sc=pd.DataFrame(scorecard); sc.to_csv(f"{SP}/scorecard_actuals.csv",index=False)
if valid_rows: pd.concat(valid_rows,ignore_index=True).to_parquet(f"{SP}/valid_preds_actuals.parquet")
json.dump(meta,open(f"{OUTDIR}/meta.json","w"),indent=0)
json.dump({f"{z}|{t}":w for (z,t),w in winners.items()},open(f"{SP}/winners_actuals.json","w"),indent=1)
print("\n=== WINNERS (NEW beats pack on VALID) ===")
for (z,t),w in sorted(winners.items()):
    print(f"  {z:12s} {t:5s} {'NEW ' if w else 'pack'}")
print("TRAIN_ACTUALS_DONE")
