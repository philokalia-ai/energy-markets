...
> The mechanism code (`src/BlockCommitment.jl`, the `:block_commitment` and
> `must_run_mode=:endogenous` wiring) and the canary harnesses live on the parked
> branch `feat/v17-block-commitment` (commits 7635e2b, 708e16f) — not on `main`.
> The standalone prototypes are committed under
